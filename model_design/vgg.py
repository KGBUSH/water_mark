...
class vgg16:
    ...
    def convlayers(self):
        ...
        with tf.name_scope('conv5_3') as scope:
            ...
            biases = tf.Variable(tf.constant(0.0, shape=[512], dtype=tf.float32),
                                 trainable=True, name='biases')
            out = tf.nn.bias_add(conv, biases)
            self.conv5_3 = tf.nn.relu(out, name=scope)  # shape=(?, 14, 14, 512)
        ...
        self.pool5 = tf.nn.max_pool(self.conv5_3,
                                    ...
                                    padding='SAME',
                                    name='pool4')
        self.layers['pool5'] = self.pool5  # shape=(?, 7, 7, 512)

    def fc_layers(self):
        # fc1
        with tf.name_scope('fc1') as scope:
            shape = int(np.prod(self.pool5.get_shape()[1:]))  # np.prod 返回给定轴上的数组元素的乘积。
        ...
        with tf.name_scope('fc3') as scope:
            ...
            self.fc3l = tf.nn.bias_add(tf.matmul(self.fc2, fc3w), fc3b)  # TensorShape([Dimension(None), Dimension(1000)])
    ...
    def load_weights(self, weight_file, sess):
        ...
        for i, k in enumerate(keys):
            ...
            sess.run(self.parameters[i].assign(weights[k]))  # tf.assign(A, new_number): 把A的值变为new_number
